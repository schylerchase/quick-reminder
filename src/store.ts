...
export class ReminderStore {
  ...
  hasPendingReminderForSourceTask(sourceTaskId: string): boolean {
    return this.pending.some((r) => r.sourceTaskId === sourceTaskId);
  }
  ...
  private async mirrorToMarkdown(): Promise<void> {
    ...
    if (existing instanceof TFile) {
      ...
      if (!isMirrorTarget) {
        this.refuseMirrorOverwrite(path);
        return;
      }
  ...
  private async isExistingMirrorFile(file: TFile): Promise<boolean> {
    try {
      ...
      return this.canOverwriteMirrorContent(content);
    } catch {
      return false;
    }
  }
  ...
  private async writeAdapterMirrorFile(
    ...
    const existingBody = await adapter.read(path);
    if (!this.canOverwriteMirrorContent(existingBody)) {
      this.refuseMirrorOverwrite(path);
      return true;
    }
  ...
  private canOverwriteMirrorContent(content: string): boolean {
    if (content.trim().length === 0) return true;
    return content.includes(MIRROR_SENTINEL);
  }

  private refuseMirrorOverwrite(path: string): void {
    console.warn(
  ...
  private async ensureConfigFolder(path: string): Promise<void> {
    ...
    const adapter = this.app.vault.adapter;
    if (await adapter.exists(REMINDER_CONFIG_DIR)) {
      return;
    }
    try {
      ...
      await adapter.mkdir(REMINDER_CONFIG_DIR);
    }
  }
